...
int main()
{
    ...
    std::vector<int> n_list = {n_elem_total};
    ...
    for (int n_elem : n_list) {
        ...
        assemble<<<blocksPerGrid, threadsPerBlock>>>(n_elem_local, n_elem_total, dx, dM0, dL0, dF0, 0);
        ...
        assemble<<<blocksPerGrid, threadsPerBlock>>>(n_elem_local, n_elem_total, dx, dM1, dL1, dF1, n_elem_local);
